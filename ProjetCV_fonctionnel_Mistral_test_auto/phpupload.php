...
try {
    ...
    var_dump($texteStructure);
    $resultatTexte = $texteStructure->choices[0]->message->content;
    // Suppression du texte ajouté par le format du schema json : " ```json " et " ``` ".
    ...
    file_put_contents($cheminJson, $resultatTexte); // Écrit les informations dans le fichier créé.
    ...
    if ($nomFichierRef) {
        ...
        try {
            ...
            if ($outputFormat === 'csv' || $outputFormat === 'both') {
                recordScoresCsv(__DIR__ . '/scores.csv', $horodate, "$nomSansExt.json", $scores['pct_ref_extr'], $scores['pct_extr_ref'], $executionTime);
            }
